#!/usr/bin/env python3
"""
January 2025 OKPK Shop Fullbay Data Ingestion Script

This script pulls January 2025 data for OKPK shop from the Fullbay API
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('january_okpk_ingestion.log')
    ]
)
...
def process_okpk_january_data():
    """Process January 2025 data for OKPK shop."""
    logger.info("🚀 Starting OKPK January 2025 Fullbay data ingestion")
    
    try:
        # Initialize components with OKPK shop configuration
        config = Config(shop_id="OKPK")
        ...
        # Test API connection
        logger.info("🔍 Testing OKPK API connection...")
        ...
        try:
            ...
            logger.info("✅ OKPK API token generation successful")
        except Exception as e:
            raise Exception(f"OKPK API connection test failed: {e}")
        
        # Generate January dates
        january_dates = generate_january_dates()
        logger.info(f"📅 Processing {len(january_dates)} days in January 2025 for OKPK")
        ...
        for i, date in enumerate(january_dates, 1):
            ...
            logger.info(f"\n📅 Processing OKPK day {i}/{len(january_dates)}: {date_str}")
            
            try:
                # Fetch invoices for this date
                logger.info(f"🔍 Fetching OKPK invoices for {date_str}...")
                ...
                if not invoices:
                    logger.info(f"ℹ️  No OKPK invoices found for {date_str}")
                    successful_days += 1  # Count as successful even if no data
                    continue
                
                logger.info(f"📊 Found {len(invoices)} OKPK invoices for {date_str}")
                
                # Insert raw data and process into line items
                logger.info(f"💾 Inserting and processing OKPK data for {date_str}...")
                records_inserted = db_manager.insert_records(invoices)
                
                logger.info(f"✅ Successfully processed {records_inserted} OKPK records for {date_str}")
                ...
                logger.error(f"❌ Failed to process OKPK {date_str}: {e}")
                failed_days += 1
                continue
        
        # Final summary
        logger.info("\n" + "="*60)
        logger.info("🎉 OKPK JANUARY 2025 INGESTION COMPLETED")
        logger.info("="*60)
        logger.info(f"✅ Successful days: {successful_days}")
        logger.info(f"❌ Failed days: {failed_days}")
        logger.info(f"📊 Total OKPK invoices processed: {total_invoices:,}")
        logger.info(f"📊 Total OKPK line items created: {total_line_items:,}")
        logger.info("="*60)
        ...
        try:
            with db_manager._get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    logger.info(f"   Total raw data records: {raw_count:,}")
                    logger.info(f"   Total line items records: {line_count:,}")
                    if date_range[0] and date_range[1]:
                        logger.info(f"   Date range: {date_range[0]} to {date_range[1]}")
                    else:
                        logger.info(f"   Date range: No valid dates found")
        except Exception as e:
            logger.warning(f"Could not retrieve final database state: {e}")
        
        return True
        
    except Exception as e:
        logger.error(f"💥 Fatal error during OKPK January ingestion: {e}")
        return False
...
def main():
    """Main function."""
    print("🚀 OKPK January 2025 Fullbay Data Ingestion")
    print("="*50)
    print("This will pull January 2025 data day by day for OKPK shop.")
    print("Each day may take up to 16+ minutes to process.")
    print("Total estimated time: 8-16 hours for the full month.")
    print()
    
    # Check if user wants to proceed
    response = input("Continue with OKPK January 2025 ingestion? (y/n): ")
    if response.lower() != 'y':
        print("❌ Operation cancelled")
        return
    
    success = process_okpk_january_data()
    
    if success:
        print("\n🎉 OKPK January 2025 ingestion completed successfully!")
        print("You can now check the results with: python check_database_state.py")
    else:
        print("\n❌ OKPK January 2025 ingestion failed!")
        print("Check the january_okpk_ingestion.log file for detailed error information.")
        sys.exit(1)
