#!/usr/bin/env python3
"""
Complete end-to-end test for June 25th data.
Pulls real data from Fullbay API, processes through pipeline, and verifies database persistence.
"""
...
def process_and_persist_data(api_data, config):
...
def query_and_verify_data(config, target_date="2025-06-25"):
...
def run_complete_test():
    ...
    client, config = test_fullbay_api_connection()
    if not client:
    ...
    if not api_data:
        ...
        raw_count, line_count = query_and_verify_data(config)
    ...
    records_inserted = process_and_persist_data(api_data, config)
